...
function rasp3 {
	...
	if test -f  ArchLinuxARM-rpi-2-latest.tar.gz
		then
			echo "Tar file allready exists"
		else
			wget http://os.archlinuxarm.org/os/ArchLinuxARM-rpi-2-latest.tar.gz
	fi
	bsdtar -xvpf ArchLinuxARM-rpi-2-latest.tar.gz -C ${SD_MOUNT}root
...
function basicSettings {
	...
	echo KEYMAP=pt-latin1 > ${SD_MOUNT}root/etc/vconsole.conf
